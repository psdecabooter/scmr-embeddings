...
CHECK_PARSED_PATH = "./tests/test_qasm/test_qasm_parsed.json"

TEST_MULTI_QREG_PATH = "./tests/multi_qreg/test_qasm.qasm"
CHECK_MULTI_QREG_PATH = "./tests/multi_qreg/test_qasm_parsed.json"
...
def test_check_files():
    ...
    assert os.path.exists(TEST_MULTI_QREG_PATH)
    assert os.path.exists(CHECK_MULTI_QREG_PATH)
...
def test_multi_qreg():
    with open(CHECK_MULTI_QREG_PATH, "r") as f:
        check_parsed = json.loads(f.read())
    check_layers = check_parsed["layers"]
    parsed_qasm = parse_qasm(TEST_MULTI_QREG_PATH)
    num_gates = len([gate for layer in parsed_qasm.layers for gate in layer])
    num_layers = len(parsed_qasm.layers)
    # Check the number of layers and gates
    assert num_gates == len([gate for layer in check_layers for gate in layer])
